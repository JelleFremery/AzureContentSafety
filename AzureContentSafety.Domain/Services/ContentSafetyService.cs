...
using AzureContentSafety.Domain.Exceptions;
using AzureContentSafety.Domain.Interfaces;
using AzureContentSafety.Domain.Requests;
using AzureContentSafety.Domain.Responses;
...
public class ContentSafetyService
{
    ...
    /// </summary>
    public static readonly string API_VERSION = "2023-04-30-preview";
    ...
    public ContentSafetyService(IOptions<ContentSafetyApiOptions> options)
    {
    ...
    public string BuildUrl(MediaType mediaType)
    {
        ...
        {
            case MediaType.Text:
                return $"{_endpoint}/contentsafety/text:analyze?api-version={API_VERSION}";
            case MediaType.Image:
                return $"{_endpoint}/contentsafety/image:analyze?api-version={API_VERSION}";
            default:
    ...
    public IDetectionRequest BuildRequestBody(MediaType mediaType, string content, string[] blocklists)
    {
        switch (mediaType)
        {
            case MediaType.Text:
    ...
    public async Task<DetectionResult> Detect(MediaType mediaType, string content, string[] blocklists)
    {
        ...
        IDetectionRequest requestBody = BuildRequestBody(mediaType, content, blocklists);
    ...
    public int? GetDetectionResultByCategory(Category category, DetectionResult detectionResult)
    {
        ...
        {
            case Category.Hate:
                return detectionResult.HateResult?.Severity;
            case Category.SelfHarm:
                return detectionResult.SelfHarmResult?.Severity;
            case Category.Sexual:
                return detectionResult.SexualResult?.Severity;
            case Category.Violence:
                return detectionResult.ViolenceResult?.Severity;
            default:
    ...
    public Decision MakeDecision(DetectionResult detectionResult, Dictionary<Category, int> rejectThresholds)
    {
        ...
        {
            if (textDetectionResult.BlocklistsMatchResults != null &&
                textDetectionResult.BlocklistsMatchResults.Count > 0)
            {
